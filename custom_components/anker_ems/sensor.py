...
def _auto_bridge_status_attrs(data: dict[str, Any]) -> dict[str, Any]:
    """Compact Action Bridge status and execution-gate diagnostics."""
    return {
...
def _auto_bridge_candidate_attrs(data: dict[str, Any]) -> dict[str, Any]:
    """Candidate-specific bridge diagnostics without duplicating slot payloads."""
    return {
        "valid": data.get("auto_bridge_valid", False),
        "reason": data.get("auto_bridge_reason"),
        "candidate_count": data.get("auto_bridge_candidate_count"),
...
def _auto_bridge_slots_attrs(data: dict[str, Any]) -> dict[str, Any]:
    """Slot-preview and Plan Store handoff diagnostics only."""
    return {
        "valid": data.get("auto_bridge_valid", False),
        "reason": data.get("auto_bridge_reason"),
        "candidate_count": data.get("auto_bridge_candidate_count"),
...
SENSORS: tuple[AnkerEmsSensorDescription, ...] = (
    ...
    AnkerEmsSensorDescription(
        ...
        attrs_fn=_auto_bridge_slots_attrs,
    ),
    AnkerEmsSensorDescription(
...
class AnkerEmsSensor(CoordinatorEntity[AnkerEmsCoordinator], SensorEntity):
    ...
    _unrecorded_attributes = frozenset({
        ...
        "prestart_diagnostic_checks",
        "prestart_test_matrix",
        "final_revalidation_checks",
        "mode_switch_preview_steps",
    })
