...
def build_planner_home_forecast(
    ...
    *,
    ...
    It remains passive; selecting it as the Plan72 source is a separate,
    ...
    for raw in quarter_rows:
        ...
        if stamp is None or stamp.minute not in {0, 15, 30, 45}:
            invalid_rows += 1
            continue
        points[stamp.replace(second=0, microsecond=0)] = value
    ...
    for offset in range(requested_hours):
        ...
        stamps = [
            hour + timedelta(minutes=15 * index)
            for index in range(QUARTERS_PER_HOUR)
        ]
        if not all(stamp in points for stamp in stamps):
            incomplete_hours.append(hour.isoformat())
            continue
        hourly.append(
            {
                "time": hour.isoformat(),
                "home_consumption_kwh": round(sum(points[stamp] for stamp in stamps), 6),
